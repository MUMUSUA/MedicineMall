<template>
  <el-dialog
    ...
    <el-form-item label="仓库id" prop="stockId">
      <el-input v-model="dataForm.stockId" placeholder="仓库id"></el-input>
    </el-form-item>
    ...
    methods: {
      init (id) {
        ...
        this.$nextTick(() => {
          ...
          if (this.dataForm.id) {
            this.$http({
              url: this.$http.adornUrl(`/stock/stockordertask/info/${this.dataForm.id}`),
              method: 'get',
              params: this.$http.adornParams()
            }).then(({data}) => {
              if (data && data.code === 0) {
                this.dataForm.orderId = data.stockOrderTask.orderId
                this.dataForm.orderSn = data.stockOrderTask.orderSn
                this.dataForm.consignee = data.stockOrderTask.consignee
                this.dataForm.consigneeTel = data.stockOrderTask.consigneeTel
                this.dataForm.deliveryAddress = data.stockOrderTask.deliveryAddress
                this.dataForm.orderComment = data.stockOrderTask.orderComment
                this.dataForm.paymentWay = data.stockOrderTask.paymentWay
                this.dataForm.taskStatus = data.stockOrderTask.taskStatus
                this.dataForm.orderBody = data.stockOrderTask.orderBody
                this.dataForm.trackingNo = data.stockOrderTask.trackingNo
                this.dataForm.createTime = data.stockOrderTask.createTime
                this.dataForm.stockId = data.stockOrderTask.stockId
                this.dataForm.taskComment = data.stockOrderTask.taskComment
              }
            })
          }
        })
      ...
      dataFormSubmit () {
        this.$refs['dataForm'].validate((valid) => {
          if (valid) {
            this.$http({
              url: this.$http.adornUrl(`/stock/stockordertask/${!this.dataForm.id ? 'save' : 'update'}`),
              method: 'post',
