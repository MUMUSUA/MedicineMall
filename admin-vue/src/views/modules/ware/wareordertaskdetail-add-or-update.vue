    ...
    methods: {
      init (id) {
        ...
        this.$nextTick(() => {
          ...
          if (this.dataForm.id) {
            this.$http({
              url: this.$http.adornUrl(`/stock/stockordertaskdetail/info/${this.dataForm.id}`),
              method: 'get',
              params: this.$http.adornParams()
            }).then(({data}) => {
              if (data && data.code === 0) {
                this.dataForm.skuId = data.stockOrderTaskDetail.skuId
                this.dataForm.skuName = data.stockOrderTaskDetail.skuName
                this.dataForm.skuNum = data.stockOrderTaskDetail.skuNum
                this.dataForm.taskId = data.stockOrderTaskDetail.taskId
              }
            })
          }
        })
      ...
      dataFormSubmit () {
        this.$refs['dataForm'].validate((valid) => {
          if (valid) {
            this.$http({
              url: this.$http.adornUrl(`/stock/stockordertaskdetail/${!this.dataForm.id ? 'save' : 'update'}`),
              method: 'post',
