  ...
  data() {
    return {
      ...
      dataRule: {
        ...
        stockId: [{ required: true, message: "仓库id不能为空", trigger: "blur" }]
      }
    };
  },
  created(){
        ...
        if (this.dataForm.id) {
          this.$http({
            url: this.$http.adornUrl(
              `/stock/purchasedetail/info/${this.dataForm.id}`
            ),
            method: "get",
            params: this.$http.adornParams()
          }).then(({ data }) => {
            if (data && data.code === 0) {
              ...
              this.dataForm.stockId = data.purchaseDetail.stockId;
    ...
    dataFormSubmit() {
      this.$refs["dataForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: this.$http.adornUrl(
              `/stock/purchasedetail/${!this.dataForm.id ? "save" : "update"}`
            ),
            method: "post",
