  ...
  >
    ...
    >
      ...
      <el-form-item label="仓库" prop="stockId">
        <el-select v-model="dataForm.stockId" placeholder="请选择仓库" clearable>
          <el-option :label="w.name" :value="w.id" v-for="w in stockList" :key="w.id"></el-option>
        </el-select>
      </el-form-item>
...
export default {
  ...
  methods: {
    getWares() {
      this.$http({
        url: this.$http.adornUrl("/stock/stockinfo/list"),
        method: "get",
    ...
    init(id) {
      ...
      this.$nextTick(() => {
        ...
        if (this.dataForm.id) {
          this.$http({
            url: this.$http.adornUrl(`/stock/stocksku/info/${this.dataForm.id}`),
            method: "get",
            params: this.$http.adornParams()
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.dataForm.skuId = data.stockSku.skuId;
              this.dataForm.stockId = data.stockSku.stockId;
              this.dataForm.stock = data.stockSku.stock;
              this.dataForm.skuName = data.stockSku.skuName;
              this.dataForm.stockLocked = data.stockSku.stockLocked;
            }
          });
        }
      });
        ...
        if (valid) {
          this.$http({
            url: this.$http.adornUrl(
              `/stock/stocksku/${!this.dataForm.id ? "save" : "update"}`
            ),
            method: "post",
