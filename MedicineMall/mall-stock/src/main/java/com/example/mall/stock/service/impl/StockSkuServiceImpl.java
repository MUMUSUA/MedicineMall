...
//    private OrderFeignService orderFeignService;
    ...
    public PageUtils queryPage(Map<String, Object> params) {
        QueryWrapper<StockSkuEntity> queryWrapper = new QueryWrapper<>();
    ...
    public void addStock(Long skuId, Long wareId, Integer skuNum) {
        ...
        //1、判读如果没有这个库存记录新增
        List<StockSkuEntity> wareSkuEntities = stockSkuDao.selectList(
                new QueryWrapper<StockSkuEntity>().eq("sku_id", skuId).eq("stock_id", wareId));

        if (wareSkuEntities == null || wareSkuEntities.size() == 0) {
            StockSkuEntity wareSkuEntity = new StockSkuEntity();
            wareSkuEntity.setSkuId(skuId);
            wareSkuEntity.setStock(skuNum);
            wareSkuEntity.setStockId(wareId);
            ...
            //添加库存信息
            stockSkuDao.insert(wareSkuEntity);
        } else {
            //修改库存信息
            stockSkuDao.addStock(skuId,wareId,skuNum);
        }
